...
export function useDragSelect({ selectedIds }: DragSelectOptions) {
  ...
  const onMouseDown = (event: MouseEvent) => {
    ...
    // Do not start drag-select if clicking inside task cards, draggable items, timeboxing view or standard interactive elements
    if (target.closest('.task-card, [draggable="true"], .timeboxing-view')) return;
