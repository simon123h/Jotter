<script lang="ts">
  ...
  import { Settings, Check, Globe } from '@lucide/svelte';
      ...
      {#each themes as th (th.id)}
        <button
          ...
          class="flex items-center gap-3 p-3.5 border rounded-xl transition-all duration-300 cursor-pointer text-left hover:scale-[1.02] {
            settingsStore.currentTheme === th.id
        ...
        >
          <span class="w-4 h-4 rounded-full shrink-0 shadow-inner {th.color}"></span>
          <div class="flex-grow">
            <div class="text-xs font-bold">{t('themeNames.' + th.id)}</div>
          </div>
          {#if settingsStore.currentTheme === th.id}
            <Check class="w-4 h-4 text-theme-primary shrink-0" />
          {/if}
        </button>
      {/each}
    </div>
  ...
  <div class="flex flex-col gap-4">
    ...
    <div class="flex flex-col gap-3">
      ...
      >
        <input
          type="checkbox"
          bind:checked={settingsStore.hideAddTaskButton}
