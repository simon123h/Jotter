<script lang="ts">
  ...
  import { Calendar, Clock, AlertCircle, ArrowRight, UserCheck, Trash } from '@lucide/svelte';
  ...
  const isUrgent = (task: Task): boolean => {
    ...
    return diffDays <= settingsStore.thresholdDays;
  };
...
<div class="flex flex-col h-full overflow-hidden select-none w-full gap-3.5">
  ...
  <div class="flex items-center justify-between bg-theme-card/35 border border-theme-border/60 rounded px-4 py-2.5 shrink-0">
    <div class="flex items-center gap-2 text-sm font-semibold text-theme-text-card">
      ...
      <span>{t('matrix.thresholdLabel', { days: settingsStore.thresholdDays })}</span>
    </div>
    <div class="flex items-center gap-1.5">
      <button
        onclick={() => updateThreshold(settingsStore.thresholdDays - 1)}
        ...
        disabled={settingsStore.thresholdDays <= 1}
      >
        -
      </button>
      <input
        type="number"
        value={settingsStore.thresholdDays}
        onchange={(e) => updateThreshold(Number((e.target as HTMLInputElement).value))}
        min="1"
  ...
  <div class="flex-grow grid grid-cols-1 md:grid-cols-2 gap-3.5 min-h-0 overflow-y-auto md:overflow-hidden p-0.5">
    ...
    <div class="flex flex-col bg-theme-column/40 border border-theme-border rounded h-full min-h-[220px] overflow-hidden md:h-full">
      ...
      <div class="flex-grow p-3 overflow-y-auto space-y-2 scroller-thin">
        {#if !quadrants.q1.length}
          <div class="h-full flex items-center justify-center text-theme-text-muted italic text-xs">
            {t('matrix.emptyQuadrant')}
          </div>
        {:else}
          {#each quadrants.q1 as task (task.id)}
            ...
            >
              ...
              <div class="flex items-center gap-2 mt-0.5 text-xs">
                ...
                {#if task.priority}
                  <span
                    class="px-1.5 py-0.25 rounded border text-[10px] font-extrabold uppercase {getPriorityClasses(task.priority)}"
                  >
                    {task.priority}
                  </span>
                {/if}
    ...
    <div class="flex flex-col bg-theme-column/40 border border-theme-border rounded h-full min-h-[220px] overflow-hidden md:h-full">
      ...
      <div class="flex-grow p-3 overflow-y-auto space-y-2 scroller-thin">
        {#if !quadrants.q2.length}
          <div class="h-full flex items-center justify-center text-theme-text-muted italic text-xs">
            {t('matrix.emptyQuadrant')}
          </div>
        {:else}
          {#each quadrants.q2 as task (task.id)}
            <!-- svelte-ignore a11y_click_events_have_key_events -->
            <!-- svelte-ignore a11y_no_static_element_interactions -->
            <div
              onclick={() => ontaskclick?.(task)}
    ...
    <div class="flex flex-col bg-theme-column/40 border border-theme-border rounded h-full min-h-[220px] overflow-hidden md:h-full">
      ...
      <div class="flex-grow p-3 overflow-y-auto space-y-2 scroller-thin">
        {#if !quadrants.q3.length}
          <div class="h-full flex items-center justify-center text-theme-text-muted italic text-xs">
            {t('matrix.emptyQuadrant')}
          </div>
        {:else}
          {#each quadrants.q3 as task (task.id)}
            <!-- svelte-ignore a11y_click_events_have_key_events -->
            <!-- svelte-ignore a11y_no_static_element_interactions -->
            <div
              onclick={() => ontaskclick?.(task)}
    ...
    <div class="flex flex-col bg-theme-column/40 border border-theme-border rounded h-full min-h-[220px] overflow-hidden md:h-full">
      ...
      <div class="flex-grow p-3 overflow-y-auto space-y-2 scroller-thin">
        {#if !quadrants.q4.length}
          <div class="h-full flex items-center justify-center text-theme-text-muted italic text-xs">
            {t('matrix.emptyQuadrant')}
          </div>
        {:else}
          {#each quadrants.q4 as task (task.id)}
            <!-- svelte-ignore a11y_click_events_have_key_events -->
            <!-- svelte-ignore a11y_no_static_element_interactions -->
            <div
              onclick={() => ontaskclick?.(task)}
